...
class DPF():
    ...
    def build_modules(self, min_obs_likelihood, proposer_keep_ratio):
        ...
        for channels in [16, 32, 64]:
            ...
            conv_layers.append(tf.nn.relu)

        # If you don't want activation after final layer, remove last one:
        # if not activate_final:
        #     conv_layers = conv_layers[:-1]

        encoder_convnet = snt.Sequential(conv_layers)
        ...
        self.mo_noise_generator = snt.Sequential([
                                    snt.Linear(32),
                                    tf.nn.relu,
                                    snt.Linear(32),
                                    tf.nn.relu,
                                    snt.Linear(self.state_dim)  # No activation after final layer
                                ], name='mo_noise_generator')
        ...
        if self.learn_odom:
            ...
            self.mo_transition_model = snt.Sequential([
                                            snt.Linear(128), tf.nn.relu,
                                            snt.Linear(128), tf.nn.relu,
                                            snt.Linear(128), tf.nn.relu,
                                            snt.Linear(self.state_dim)  # Final layer, no activation
                                        ], name='mo_transition_model')
    ...
    def motion_update(self, actions, particles, means, stds, state_step_sizes, stop_sampling_gradient=False):
        ...
        random_input = tf.random.normal(tf.shape(action_input))
        ...
        delta -= tf.reduce_mean(delta, axis=1, keepdims=True)
    ...
    def compile_training_stages(self, sess, batch_iterators, particle_list, particle_probs_list, encodings, means, stds, state_step_sizes, state_mins, state_maxs, learning_rate, plot_task):
        ...
        if self.learn_odom:
            ...
            var_list = [v for v in tf.compat.v1.get_collection(tf.compat.v1.GraphKeys.GLOBAL_VARIABLES) if 'mo_transition_model' in v.name]
        ...
        optimizer = tf.compat.v1.train.AdamOptimizer(learning_rate=learning_rate)
        var_list = [v for v in tf.compat.v1.get_collection(tf.compat.v1.GraphKeys.GLOBAL_VARIABLES) if 'mo_noise_generator' in v.name]
        ...
        optimizer = tf.compat.v1.train.AdamOptimizer(learning_rate=learning_rate)
        var_list = [v for v in tf.compat.v1.get_collection(tf.compat.v1.GraphKeys.GLOBAL_VARIABLES) if 'encoder' in v.name or 'obs_like_estimator' in v.name]
        ...
        if self.use_proposer:
            ...
            optimizer = tf.compat.v1.train.AdamOptimizer(learning_rate=learning_rate)
            var_list = [v for v in tf.compat.v1.get_collection(tf.compat.v1.GraphKeys.GLOBAL_VARIABLES) if 'particle_proposer' in v.name]
    ...
    def load(self, sess, model_path, model_file='best_validation', statistics_file='statistics.npz', connect_and_initialize=True, modules=('encoder', 'mo_noise_generator', 'mo_transition_model', 'obs_like_estimator', 'particle_proposer')):
        ...
        saver = tf.compat.v1.train.Saver(vars_to_load)  # <- var list goes in here
